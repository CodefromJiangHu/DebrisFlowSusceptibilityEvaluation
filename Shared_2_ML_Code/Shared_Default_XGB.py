...
def optimalizing_PSO_XGB(p):
    ...
    model = aimCL.get_xgboost_GridSearch()
...
if __name__ == '__main__':
    ...
    modelNmae = "XGB"
    flagSimle = False
    flagUseGridSearchCV = False
    flagExplainImportance_Shap = False
    flagExplainImportance_Eli5 = False
    flag_PSO = False
    flag_Defalt = True
    n_splits = 10
    n_estimators_optimal, max_depth_optimal, lr_optimal = int(569.6671), int(126.0709), 0.3083
    str_para_vary = str(n_estimators_optimal) + "_" + str(max_depth_optimal) + "_" + str(lr_optimal)
    if flag_Defalt:
        str_para_vary = "参数默认_"
    ...
    if flag_PSO:
        ...
        pso = PSO(func=optimalizing_PSO_XGB, n_dim=3, pop=20, max_iter=150, lb=[30, 2, 1.0e-8], ub=[1000, 30, 1], w=0.8, c1 = 0.5, c2 = 0.5)
        ...
        n_estimators_optimal, max_depth_optimal, lr_optimal = int(pso.gbest_x[0]), int(pso.gbest_x[1]), round(pso.gbest_x[2], 4)
        # optimal parameters
        str_para_vary = str(n_estimators_optimal) + "_" + str(max_depth_optimal)+ "_" + str(lr_optimal)
        writeMessage("最优参数为：" + str(str_para_vary), "outputs/" + modelNmae + "_优化器参数调试结果.txt")
        plt.plot(pso.gbest_y_hist)
        plt.show()

    if flag_evaluation:
        # obtain model
        model = aimCL.get_Trained_XGB_GridSearch(X_train, Y_train, n_estimators_optimal, max_depth_optimal, lr_optimal, n_splits)
